...
fit <- randomForest(as.factor(count) ~ season + holiday + weather + dow + hour + temp + humidity + windspeed , data=train, ntree = 250, importance=TRUE)
varImpPlot(fit)
...
write.csv(output, file = "randomforestOutput.csv", row.names = FALSE)
